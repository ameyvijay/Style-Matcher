...
export function useImageCache(photos, topIndex, syncMode = "optimal") {
  ...
  const prefetchLevel = useCallback(
    async (photo, level, size, priority) => {
      ...
      if (!objectURL) {
        // Log as warning only to avoid intrusive Next.js error overlay.
        // The UI will fall back to the raw URL in getObjectURL.
        console.warn(`[useImageCache] Pre-fetch failed (likely CORS): ${url}`);
      }
      inflight.current.delete(key);
  ...
  const getObjectURL = useCallback((photoId) => {
    const entry = cache.current.get(photoId);
    if (!entry) {
        // Fallback: If no cache entry exists, return null so SwipeCard 
        // can fall back to the raw URL (rlhf_url)
        return null;
    }
