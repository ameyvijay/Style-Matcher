  ...
  async analyzeStyle(base64Image, options = {}) {
    ...
    try {
      ...
      if (parsedUrl.hostname !== "localhost" && parsedUrl.hostname !== "127.0.0.1") {
        throw new Error("Invalid ollamaUrl: Only localhost is permitted for local VLM execution.");
      }
      safeOllamaUrl = parsedUrl.origin;
    } catch (e) {
      throw new Error(`SSRF Prevention blocked request: ${e.message}`);
    }
