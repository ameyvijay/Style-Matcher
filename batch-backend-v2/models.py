...
# ─── Quality Scoring ─────────────────────────────────────────────────
@dataclass
class DiscordanceFlag:
    """Attached to QualityScore when expert signals disagree beyond threshold."""
    triggered: bool = False
    expert_pair: str = ""          # "vlm_vs_sharpness", "vlm_vs_exposure", etc.
    delta: float = 0.0            # Absolute difference
    vlm_score: float = 0.0
    frozen_score: float = 0.0
    action: str = ""              # "clamp", "flag_review", "override_to_cull"
...
class QualityScore:
    ...
    tier: Tier = Tier.CULL
    recovery_potential: str = ""  # "high", "medium", "low", "discordance_review"
    processing_time: float = 0.0  # Seconds
    semantic_description: str = "" # Generated by Ollama Vision
    semantic_similarity: float = 0.0 # Similarity to user preference RAG (VLM Preference)
    
    # ── MoE Weight Audit ───────────────────────────────────────────
    vlm_trust_tau: float = 0.0
    weights_used: dict = None      # Stores {w_sharp, w_aesthetic, w_exposure, w_vlm}
    discordance: Optional[DiscordanceFlag] = None
    ...
    def to_dict(self):
        ...
        d = {
            ...
            "weights": {
                "tau": self.vlm_trust_tau,
                **(self.weights_used or {})
            }
        }
        if self.discordance and self.discordance.triggered:
            d["discordance"] = {
                "expert_pair": self.discordance.expert_pair,
                "delta": self.discordance.delta,
                "action": self.discordance.action,
                "vlm_score": self.discordance.vlm_score,
                "frozen_score": self.discordance.frozen_score
            }
        return d
